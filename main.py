...
def mark_done():
    ...
    while mark < 1 or mark > len(todo_list_tasks):
        ...
        if mark < 1 or mark > len(todo_list_tasks):
            print ("Invalid task number")
        if todo_list_tasks[mark-1]['done']:
...
def edit_task():
    ...
    display_tasks()
    edit = 0
    while edit < 1 or edit > len(todo_list_tasks):
        try:
            edit = int(input('Enter a number of task to edit:'))
        except ValueError:
            print('Invalid number, please try again.')
            continue
        if edit < 1 or edit > len(todo_list_tasks):
            print ("Invalid task number")
    ...
    for task in todo_list_tasks:
        if task['title'] == new_task:
            ...
            return
    todo_list_tasks[edit-1]['title'] = new_task
    display_tasks() 
def show_menu():
    print('1 - Add a task\n2 - Display tasks\n3 - Remove a task\n4 - Mark task as done\n5 - Edit a task\n6 - Exit')      
    while True:
        ...
        if choice == 1:
            ...
            save_tasks()
            print('Tasks saved. Exiting the programm...')
            break
        else:
